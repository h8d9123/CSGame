...
void CCamera::Pitch(float fAngle)//ÉÏÏòÁ¿Æ«º½½Ç
{
...
void CCamera::Strafe(float fDistance)
{
	if (m_CameraType == LANDCAMERA)
...
void CCamera::Fly(float fDistance)
{
	if (m_CameraType == LANDCAMERA)
		m_vPos.y += fDistance;
	else
...
void CCamera::Walk(float fDistance)
{
	if (m_CameraType == LANDCAMERA)
...
void CCamera::GetViewMatirx(D3DXMATRIXA16 *pMatView)
{
